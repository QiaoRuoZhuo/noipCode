...
int main( )
{
	...
	//整理数据
	for (int j=0, i=na; i<n; i++,j++)
	{
		libA[i] = libA[j];
	}
	for (int j=0, i=nb; i<n; i++,j++)
	{
		libB[i] = libB[j];
	}
	...
	for (int i=0; i<n; i++)
	{
		if ((libA[i] == 0 && libB[i] == 2) || (libA[i] == 2 && libB[i] == 5) || (libA[i] == 5 && libB[i] == 0)) //A赢 
		{
			sA++;
		}
		else if ((libB[i] == 0 && libA[i] == 2) || (libB[i] == 2 && libA[i] == 5) || (libB[i] == 5 && libA[i] == 0))  //B赢  
		{
			sB++;
		}
	}
