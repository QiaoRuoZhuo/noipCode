...
void Backtrace_2() //非递归回溯 
{
    bool flag[N] = {0}; //标记是否曾经将结点X[P[t]]换到下标为t处
    int P[N]; //存储t结点的下标 
    for (int i=0; i<N; i++) //设置初始的顶点顺序 
	{
	 	P[i] = i;
	...
	while(t >= 0)
	{
		while(P[t] < N)//c[t]默认初始化为0 
		{
	        flag[P[t]] = false;
			if (OK(X[t-1], X[P[t]]))//如果满足约束条件，则分析X[P[t]]结点 
			{
                flag[P[t]] = true;
                Swap(X[t], X[P[t]]); //先把满足条件的结点X[P[t]]换到下标为t处 
				d += map[X[t-1]][X[t]]; 
				if(t == N-1)
				{
					if(OK(X[t], X[0]))
		...
		if (flag[P[t]-1])//如果满足约束条件，则还原X[P[t]]结点 
		{
			d -= map[X[t-1]][X[t]];
			Swap(X[t], X[P[t]-1]);
		}
